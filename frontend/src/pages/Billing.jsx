...
const Billing = () => {
  ...
  const fetchMetadata = async () => {
    try {
      ...
      setDoctors(
        Array.isArray(docRes.data) ? docRes.data : docRes.data.results,
      );
      setPatients(
        Array.isArray(patRes.data) ? patRes.data : patRes.data.results,
      );
      setPharmacyStock(
        Array.isArray(stockRes.data) ? stockRes.data : stockRes.data.results,
      );
      setServiceDefinitions(
        Array.isArray(svcRes.data) ? svcRes.data : svcRes.data.results,
      );
      setAvailableLabTests(
        Array.isArray(labRes.data) ? labRes.data : labRes.data.results,
      );
    } catch (err) {
      console.error(err);
    }
  ...
  const handleBillNow = async (visit) => {
    ...
    const patientName =
      visit.patient_name || (patientObj ? patientObj.full_name : "Unknown");
    ...
    if (!doctorToSet && visit.doctor_name) {
      const foundDoctor = doctors.find((d) =>
        `Dr. ${d.first_name} ${d.last_name}`
          .toLowerCase()
          .includes(visit.doctor_name.toLowerCase()),
      );
    ...
    try {
      const existingInvoiceRes = await api.get(
        `/billing/invoices/?visit=${visit.id || visit.v_id}`,
      );
      const existingInvoices =
        existingInvoiceRes.data.results || existingInvoiceRes.data;
      const activeInvoice = existingInvoices.find(
        (inv) => inv.payment_status !== "CANCELLED",
      );
    ...
    const isPharmacyVisit =
      visit.vitals &&
      visit.vitals.note === "Auto-created from Pharmacy Manual Sale";
    ...
    const isCasualtyDirectLab =
      visit.assigned_role === "LAB" &&
      (visit.casualty_services?.length > 0 ||
        visit.casualty_medicines?.length > 0);
        ...
        if (obs.is_active || obs.end_time) {
          ...
          if (obs.start_time) {
            ...
            durationMinutes = Math.max(
              elapsed,
              obs.planned_duration_minutes || 0,
              60,
            );
          }
          ...
          const obsService = serviceDefinitions.find((s) =>
            s.name.toLowerCase().includes("observation"),
          );
          const chargePerHr = obsService
            ? parseFloat(obsService.base_charge)
            : 500;
          ...
          newFormData.items.push({
            dept: "CASUALTY",
            description: `Observation Charges (${hours} hrs @ ₹${chargePerHr}/hr)`,
            qty: 1,
            unit_price: obsAmount,
            amount: obsAmount,
            hsn: "",
            batch: "",
            gst_percent: 0,
  ...
  const handleImportPrescription = async (overridePatientId = null) => {
    ...
    try {
      const vId =
        typeof formData.visit === "object" ? formData.visit.id : formData.visit;

      // 1. Fetch Doctor Notes
      let notesData = [];
      try {
        const res = vId
          ? await api.get(`medical/doctor-notes/?visit=${vId}`)
          : await api.get(`medical/doctor-notes/?visit__patient=${patId}`);
        notesData =
      ...
      if (vId) {
        try {
          ...
          console.warn(
            "Failed to fetch fresh visit details, utilizing fallback:",
            e,
          );
      ...
      const newItems = [...formData.items];
      const addedMedNames = new Set(
        newItems.map((i) => i.description.toLowerCase()),
      );
      ...
      if (notesData.length > 0) {
        ...
        if (lastNote.prescription) {
          const presItems = Array.isArray(lastNote.prescription)
            ...
            : Object.entries(lastNote.prescription).map(([name, details]) => ({
                name,
                details,
              }));
          ...
          presItems.forEach((p) => {
            ...
            const pharmacyRecord = freshPharmacySales.find(
              (i) => i.name.toLowerCase() === medName.toLowerCase(),
            );
            const stockItem = pharmacyStock.find(
              (s) => s.name.toLowerCase() === medName.toLowerCase(),
            );
  ...
  const handleCreateInvoice = async (status = "PENDING") => {
    ...
    try {
      ...
      if (validItems.length === 0) {
        showToast(
          "error",
          "Please add at least one item to generate an invoice.",
        );
        return;
      }
      ...
      for (const item of validItems) {
        if (item.dept === "PHARMACY" && !item.stock_deducted) {
          ...
          const stock = pharmacyStock.find(
            (s) =>
          ...
          if (stock) {
            if (stock.qty_available < item.qty) {
              return showToast(
                "error",
                `Insufficient stock for ${item.description}. Available: ${stock.qty_available}, Requested: ${item.qty}`,
              );
            }
          } else if (item.qty > 0) {
            return showToast(
              "error",
              `No stock record found for ${item.description}. Please select from the dropdown.`,
            );
          }
        }
      }
      ...
      if (formData.visit) {
        const visitId =
          typeof formData.visit === "object"
            ? formData.visit.id
            : formData.visit;
        if (visitId) {
          const newLabTests = validItems
            .filter((i) => i.dept === "LAB" && i.ref_id)
            .map((i) => i.ref_id);
      ...
      if (status === "DRAFT") {
        ...
        if (
          savedInvoice &&
          (savedInvoice.payment_status === "PENDING" ||
            savedInvoice.payment_status === "PARTIAL")
        ) {
          setPaymentData({
  ...
  const handleEditInvoice = async (invoice) => {
    ...
    try {
      const vId =
        invoice.visit && typeof invoice.visit === "object"
          ? invoice.visit.id
          : invoice.visit;
      let visitData = null;
      ...
      baseItems = baseItems.filter(
        (i) => !(i.dept === "LAB" && parseFloat(i.amount) === 0),
      );

      // Sync Pharmacy Items
      const visitPharmacyItems = (
      ...
      const visitCasualtyServices = (
        (visitData && visitData.casualty_services) ||
        []
      ).map((item) => ({
        dept: "CASUALTY",
        description: item.name,
      ...
      if (
        ...
        const existingKeys = new Set(
          baseItems.map((i) => `${i.description}-${i.batch || ""}`),
        );
        uniquePharmacyItems = visitPharmacyItems.filter(
          (i) => !existingKeys.has(`${i.description}-${i.batch || ""}`),
        );
        uniqueCasualtyMeds = visitCasualtyMedicines.filter(
          (i) => !existingKeys.has(`${i.description}-${i.batch || ""}`),
        );
        uniqueCasualtyServices = visitCasualtyServices.filter(
          (i) => !existingKeys.has(`${i.description}-`),
        );
        uniqueLabItems = visitLabItems.filter(
          (i) => !existingKeys.has(`${i.description}-`),
        );
      }
      ...
      setFormData({
        ...
        doctor_display_name:
          invoice.doctor_display_name ||
          (visitData ? visitData.doctor_name : "") ||
          "Not Assigned",
        payment_status: invoice.payment_status,
        discount_amount: invoice.discount_amount || 0,
        registration_number:
          invoice.registration_number ||
          (visitData && visitData.patient
      ...
      if (invoice.patient_id || (visitData && visitData.patient)) {
        setSelectedPatientId(
          invoice.patient_id ||
            (visitData && visitData.patient?.id) ||
            visitData.patient,
        );
      }
      setShowModal(true);
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      await api.post(
        `billing/invoices/${paymentData.invoice.id}/add_payment/`,
        {
  ...
  const StatusBadge = ({ status }) => {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wide border shadow-sm ${isPaid ? "bg-emerald-50 text-emerald-700 border-emerald-100" : "bg-amber-50 text-amber-700 border-amber-100"}`}
      >
  ...
  const filteredPendingVisits = pendingVisits.filter(
    (visit) => !unpaidInvoiceVisitIds.has(visit.id),
  );
  ...
  return (
    <div className="p-6 md:p-8 max-w-[1600px] mx-auto min-h-screen bg-[#F8FAFC] font-sans text-slate-900 relative print:p-0 print:m-0 print:min-h-0 print:bg-white print:overflow-visible print:max-w-none">
        ...
        {unifiedQueue.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 p-5">
            {unifiedQueue.map((item) => (
              ...
              >
                <div
                  className={`absolute top-0 right-0 w-16 h-16 rounded-bl-[3rem] -mr-8 -mt-8 z-0 ${item.queue_type === "invoice" ? "bg-orange-50" : "bg-blue-50"}`}
                ></div>
                <div className="relative z-10">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-bold text-slate-900 text-sm line-clamp-1">
                          {item.patient_name || "Guest"}
                        </h4>
                        <p className="text-[10px] font-bold text-slate-400 uppercase">
                          {item.queue_type === "invoice"
                            ? `INV: ${item.invoice_number || item.id}`
                            : `VISIT: ${item.id}`}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-1 mt-3">
                      ...
                      parseFloat(item.consultation_fee) > 0 ? (
                        <div className="flex justify-between text-xs">
                          ...
                          <span className="font-bold text-slate-700">
                            ₹{parseFloat(item.consultation_fee).toFixed(2)}
                          </span>
                        </div>
                      ) : item.doctor ? (
                        <div className="flex justify-between text-xs">
                      ...
                      {item.lab_charges_data &&
                        item.lab_charges_data.length > 0 && (
                          <div className="flex justify-between text-xs">
                            <span className="text-slate-500 font-medium">
                              Lab Tests
                            </span>
                      ...
                      {item.casualty_services &&
                        item.casualty_services.length > 0 && (
                          <div className="flex justify-between text-xs">
                            <span className="text-slate-500 font-medium">
                              Services
                            </span>
                            <span className="font-bold text-slate-700">
                              ...
                              {item.casualty_services
                                .reduce(
                                  (sum, i) =>
                                    sum + parseFloat(i.total_charge || 0),
                                  0,
                                )
                                .toFixed(2)}
                            </span>
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-100 flex justify-between items-center">
                    {item.queue_type === "invoice" ? (
                      <span
                        className={`text-[10px] font-black px-2 py-1 rounded ${item.payment_status === "PARTIAL" ? "text-orange-600 bg-orange-50" : "text-rose-600 bg-rose-50"}`}
                      >
                        {item.payment_status}
                      </span>
                    ) : (
          ...
          <div className="bg-white border-2 border-dashed border-slate-200 rounded-2xl p-8 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-xs text-slate-500 mt-1 max-w-[200px]">
              Patients will appear here when they are marked for billing.
            </p>
          </div>
        )}
      </div>
                        ...
                        {invoice.payments && invoice.payments.length > 0 ? (
                          <div className="flex flex-col gap-0.5 mt-1">
                            {invoice.payments.map((p, pIdx) => (
                              <span
                                key={pIdx}
                                className="text-[9px] font-bold text-slate-500 uppercase flex items-center gap-1"
                              >
                                {p.mode === "CASH" && <Wallet size={10} />}
                                {p.mode === "UPI" && <Sparkles size={10} />}
                                {p.mode === "CARD" && <CreditCard size={10} />}
                                {p.mode === "UPI" ? "GooglePay" : p.mode}: ₹
                                {p.amount}
                              </span>
                            ))}
                          </div>
                    ...
                    <td className="px-6 py-4 text-right flex justify-end gap-2">
                      ...
                      {invoice.payment_status !== "PAID" && (
                        <button
                          ...
                          className="text-emerald-500 hover:text-emerald-700 p-2 rounded-lg hover:bg-emerald-50 border border-emerald-100"
                        >
                          <CreditCard size={16} />
                        </button>
                      )}
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2 px-3 py-1.5 bg-purple-100/50 rounded-lg text-purple-700 border border-purple-200">
                ...
                <span className="text-xs font-bold">
                  Card: ₹
                  {(stats?.monthly_breakdown?.CARD || 0).toLocaleString()}
                </span>
              </div>
            </div>
          </div>
        ...
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm print-modal">
            <motion.div
              ...
              className="bg-white w-full max-w-7xl rounded-[2rem] shadow-2xl overflow-hidden flex flex-col h-[95vh] no-print print:hidden"
            >
              {/* Modal Header */}
                ...
                <div className="grid grid-cols-2 gap-12 mb-12">
                  <div>
                    ...
                    <div className="text-xs text-slate-500 mt-1">
                      Reg No: {formData.registration_number || "N/A"}
                    </div>
                  </div>
                  <div className="text-right">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">
                ...
                <div className="mb-12 overflow-visible min-h-[400px]">
                  <table className="w-full text-left text-sm min-w-[1000px]">
                    <thead className="border-b-2 border-slate-900">
                      <tr>
                        <th className="py-3 text-[10px] font-black text-slate-900 uppercase tracking-widest w-12">
                          #
                        </th>
                        <th className="py-3 text-[10px] font-black text-slate-900 uppercase tracking-widest min-w-[250px]">
                          Description
                        </th>
                        <th className="py-3 text-[10px] font-black text-slate-900 uppercase tracking-widest w-16 text-center">
                          Qty
                        </th>
                        <th className="py-3 text-[10px] font-black text-slate-900 uppercase tracking-widest w-16 text-center">
                          GST%
                        </th>
                        <th className="py-3 text-[10px] font-black text-slate-900 uppercase tracking-widest w-24 text-right">
                          Price
                        </th>
                        <th className="py-3 text-[10px] font-black text-slate-900 uppercase tracking-widest w-28 text-right">
                          Amount
                        </th>
                      ...
                      {formData.items.map((item, idx) => (
                        <tr key={idx} className="group">
                          ...
                          <td
                            ...
                            style={{
                              zIndex: stockSearch.index === idx ? 100 : 1,
                            }}
                          >
                            <input
                              readOnly={
                              ...
                              stockSearch.results.length > 0 && (
                                <div className="absolute top-full left-0 z-[100] w-[400px] bg-white border border-slate-200 rounded-xl shadow-2xl py-2 mt-2 max-h-[300px] overflow-y-auto">
                                  {stockSearch.results.map((item) => {
                                    ...
                                    const outOfStock =
                                      !isService && item.qty_available <= 0;
                                    ...
                                    return (
                                      ...
                                      >
                                        <div>
                                          <p
                                            className={`text-sm font-bold flex items-center gap-2 ${outOfStock ? "text-slate-400" : "text-slate-800"}`}
                                          >
                                            {item.name}
                                            {isService && (
                                              <span className="text-[8px] px-1.5 py-0.5 rounded bg-purple-100 text-purple-700 uppercase tracking-widest">
                                                Service
                                              </span>
                                            )}
                                          </p>
                                          {!isService && (
                                            <div className="flex items-center gap-2 mt-0.5">
                                              ...
                                              <span
                                                className={`text-[10px] font-bold ${outOfStock ? "text-rose-500" : "text-emerald-500"}`}
                                              >
                                                {outOfStock
                                                  ? "Out of Stock"
                                        ...
                                        <div className="text-right">
                                          <p
                                            className={`text-xs font-black ${outOfStock ? "text-slate-400" : "text-slate-900 group-hover:text-blue-600"} transition-colors`}
                                          >
                ...
                <div>
                  {(!formData.id || formData.payment_status === "DRAFT") && (
                    <button
                      ...
                      className={`text-xs font-bold flex items-center gap-2 ${selectedPatientId ? "text-blue-600 hover:text-blue-800" : "text-slate-300 cursor-not-allowed"}`}
                    >
                ...
                <div className="flex gap-3">
                  ...
                  {(!formData.id || formData.payment_status === "DRAFT") && (
                    <button
                      ...
                      className="px-6 py-3 rounded-xl font-bold text-blue-600 hover:bg-blue-50 border border-transparent hover:border-blue-200 transition-all"
                    >
                      Save as Draft
                    </button>
                  )}
                  {(!formData.id ||
                    ...
                    <button
                      ...
                      disabled={isSubmitting}
                      className="px-8 py-3 bg-slate-900 text-white rounded-xl font-bold shadow-xl shadow-slate-900/20 hover:bg-blue-600 transition-all flex items-center gap-2 disabled:bg-slate-400 disabled:cursor-not-allowed"
                    >
      ...
      <div
        ...
        className="hidden print:block absolute top-0 left-0 w-full h-auto bg-white z-[9999] p-8"
      >
          ...
          <div>
            ...
            <div className="flex justify-between items-start mb-8 border-b-2 border-slate-900 pb-4">
              <div className="flex items-center gap-4">
                ...
                />
                <div>
                  <h1 className="text-3xl font-black text-slate-900 tracking-widest uppercase">
                    ANOVA MEDICAL CENTER
                  </h1>
              ...
              <div className="text-right">
                ...
                <p className="text-sm font-bold text-slate-900 mt-2">
                  #{formData.id ? String(formData.id).toUpperCase() : "DRAFT"}
                </p>
            ...
            <table className="w-full text-left border-collapse border border-slate-900">
              ...
              <tbody>
                <tr className="bg-slate-50 border-t-2 border-slate-900">
                  ...
                  <td className="py-2 px-2 text-right text-sm font-bold text-slate-600">
                    ₹{calculateSubtotal(formData.items).toFixed(2)}
                  </td>
                </tr>
                {parseFloat(formData.discount_amount) > 0 && (
                  <tr className="bg-rose-50 border-t border-slate-300">
                    ...
                    <td className="py-2 px-2 text-right text-sm font-bold text-rose-600">
                      -₹{parseFloat(formData.discount_amount).toFixed(2)}
                    </td>
                  </tr>
                )}
          ...
          <div>
            <div className="flex flex-col items-center mt-8 pt-2">
              <div className="text-[9px] text-slate-500 max-w-lg text-center leading-relaxed mb-8">
                ...
                <p>
                  Certified that the medicines sold as per this bill have been
                  purchased local from Registered Dealers who have certified in
                  the related sales bills that such medicines had duly suffered
                  compound tax. Wish You a Speedy Recovery.
                </p>
              </div>
              ...
              <div className="p-6 space-y-6">
                <div className="text-center">
                  ...
                  <p className="text-4xl font-black text-slate-900 mt-1">
                    ...
                    {Math.max(
                      0,
                      parseFloat(paymentData.invoice.total_amount) -
                        (parseFloat(paymentData.invoice.discount_amount) || 0),
                    ).toFixed(2)}
                  </p>
                  {paymentData.invoice.amount_paid > 0 && (
                    <p className="text-xs font-bold text-emerald-600 mt-1">
                      Paid: ₹{paymentData.invoice.amount_paid} • Due: ₹
                      {paymentData.invoice.balance_due}
                    </p>
                  )}
                </div>
                  ...
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <span className="text-xs font-bold text-slate-400">
                      ...
                      <span className="text-emerald-600">
                        ₹
                        {Object.values(paymentData.payments)
                          .reduce(
                      ...
                      <span className="text-red-500">
                        ...
                        {Math.max(
                          ...
                          parseFloat(
                            ...
                            Object.values(paymentData.payments).reduce(
                              (acc, curr) => acc + (parseFloat(curr) || 0),
                              0,
                            ),
                        ).toFixed(2)}
